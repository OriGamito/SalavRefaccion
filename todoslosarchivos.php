...
class MySqlBackupLite
{
    ...
    public function __construct($arrayConnConfig)
    {
    ...
    public function zipcreation($usuario, $sucursal, $arrayDbConf)
    {
        ...
        try {
            ...
            $mySqli = new mysqli($arrayDbConf['host'], $arrayDbConf['user'], $arrayDbConf['pass'], $arrayDbConf['name']);
            $mySqli->select_db($arrayDbConf['name']);
            ...
            //QUIMICOS INVENTARIO
            $tabla = "SELECT count(id) FROM `inventarioosquimicos$suc` WHERE sucursal_id=$suc";
            $cantijQuimicoInv = $mySqli->query("SELECT count(id) FROM `inventarioosquimicos$suc` WHERE sucursal_id=$suc");

            $cantidadhcjQuiInventario = $cantijQuimicoInv->fetch_row();
            $cantidadInveQuimico = $cantidadhcjQuiInventario[0];
            $cantidadQInv = ceil($cantidadInveQuimico) / 100000;

            $canidaincrementablequiInv = 0;

            for ($i = 0; $i < $cantidadQInv; $i++) {
                $inventarioll = "quimicoinventario";
                $bck = new MySqlBackupLite($arrayDbConf);
                $bck->backUp($canidaincrementablequiInv, $suc, $inventarioll);
                $bck->setFileDir('');
                $canidaincrementablequiInv = $canidaincrementablequiInv + 100000;
                $bck->setFileName('backupFilequimicoinv' . $i + 1 . '.txt');
                $nombresdescript[$iuni] = 'backupFilequimicoinv' . $i + 1 . '.txt';
                $bck->saveToFile();
                $contadordenombres++;
                $iuni++;
            }
    ...
    private function getTableData($table, $numjk, $suc, $inventarioll)
    {
        ...
        if ($inventarioll == "quimicoinventario") {

            $this->queryResult    = $this->mySqli->query("SELECT * FROM inventarioosquimicos$suc  limit  $ofset,100000");
            $this->tableFieldCount = $this->queryResult->field_count;
            $this->tableNumberOfRows = $this->mySqli->affected_rows;
        }
    ...
    private function insertResultHeader($table, $currentRowNumber, $inventarioll, $suc)
    {
        ...
        if ($inventarioll == "quimicoinventario") {
            ...
            if ($currentRowNumber % 10000 == 0 || $currentRowNumber == 0) {
                $this->sqlString .= "INSERT INTO inventarioosquimicos VALUES";
            }
        }
        if ($inventarioll == "consulta") {
            if ($currentRowNumber % 10000 == 0 || $currentRowNumber == 0) {
